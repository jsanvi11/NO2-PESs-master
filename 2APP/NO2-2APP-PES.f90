...
real*8, parameter :: wtol = epsilon(1.0d0), dr1 = 1.40d0, dr2 = 1.35d0, dr3 = 1.35d0 
...
if (.not. ker1) then
  inquire(file=trim(datapath)//"pes31.kernel", exist=ker1)   ! file_exists will be true if the file exists and false otherwise
end if

if (.not. ker2) then
  inquire(file=trim(datapath)//"pes32.kernel", exist=ker2)   ! file_exists will be true if the file exists and false otherwise
end if

!if (.not. ker3) then
!  inquire(file=trim(datapath)//"pes33.kernel", exist=ker3)   ! file_exists will be true if the file exists and false otherwise
!end if
...
open(unit=1001,file=trim(datapath)//"asymp3.dat", status = "old")
...
if (ker1 .and. ker2 ) then
  call pes31%load_from_file(trim(datapath)//"pes31.kernel")
  call pes32%load_from_file(trim(datapath)//"pes32.kernel")
!  call pes33%load_from_file(trim(datapath)//"pes33.kernel")
  kread = .true.
else
  call pes31%read_grid(trim(datapath)//"pes31.csv")
!  print*,"IAMHERE"
...
!
  call pes31%save_to_file(trim(datapath)//"pes31.kernel")
!
  call pes32%read_grid(trim(datapath)//"pes32.csv")
!  print*,"IAMHERE"
...
!
  call pes32%save_to_file(trim(datapath)//"pes32.kernel")

!  call pes33%read_grid(trim(datapath)//"pes33.csv")
!  print*,"IAMHERE"
...
!
!  call pes33%save_to_file(trim(datapath)//"pes33.kernel")
  kread = .true.
end if
end if
...
subroutine no22apppes(tmpr, totener, dvdr)
use surface3
implicit none
